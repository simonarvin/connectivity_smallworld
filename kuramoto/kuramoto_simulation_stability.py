...
def kuramoto(G, coupling, coherence):

    print("instantiating kuramoto")
    graph = nx.to_numpy_array(G)

    # Instantiate model with parameters
    model = Kuramoto(coupling=coupling, dt=kuramoto_dt, T=kuramoto_time, n_nodes=len(graph), coherence = coherence)
    print("success")
...
kuramoto_dt = .1
kuramoto_time = 25#100 # * kuramoto_dt

couplings = [3]#[4, 3, 2, 1]#, 2.5]
couplings = couplings[start_at_coupling:]
coherences = np.linspace(0, 1, 10)#[1:]

#instantiate experimental parameters
total_trials_samples = 15
sample_range = range(total_trials_samples)
total_trials = len(shortrange_pernode) * len(longrange_probabilities) * len(max_longrange) * len(coherences)
...
for coupling in couplings:
    ...
    file_name = f"{base_path}/sw_kuramoto_coh3_coupling{coupling}_{timestr}.json"
                ...
                for u in longrange_probabilities:
                    ...
                    try:
                        ...
                        for sample in sample_range: #run samples
                            ...
                            phase_cohs.append(kuramoto(G, coupling, coherence))
